...
//    LOGE("准备解码")
    while (agrs->isPlay){
        size=0;
        int ret = agrs->get(avPacket);

        if (ret == 2) { //数据队列没数据并且已经读完原数据了.
            break;
        }
...
int FFmpegMusic::get(AVPacket *avPacket) {
    ...
    pthread_mutex_lock(&mutex);
    while (isPlay){
        if(!queue.empty()&&isPause){ //有数据并且没被暂停
            //如果队列中有数据可以拿出来
        ...
        if (queue.empty() && isReadResourceDone) {//没数据并且已经读完原数据了.
            ret = 2;
            break;
        }
